...
import {getMovieVideos, MovieVideoType} from './TMDB';
...
function MovieDetailsScreen(props): JSX.Element {
  ...
  const [movieVideos, setMovieVideos] = useState<MovieVideoType[]>([]);

  useEffect(() => {
    getMovieVideos(movie).then(videos => {
      ...
      <View style={styles.movieDetails}>
        ...
        {movieVideos.map((video, idx) => (
          <TouchableHighlight
            key={idx}
            onPress={() => {
              navigation.navigate('MovieVideo', {video: video});
            }}>
            <Text style={[styles.movieItemText, {color: '#00bbaa'}]}>{idx+1}. <Text style={{color: 'orange'}}>{video.type}</Text> {video.name}</Text>
          </TouchableHighlight>
        ))}
      </View>
      </View>
